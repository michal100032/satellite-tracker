...
const double DATA_UPDATE_INTERVAL = 86400.0;

OrbitData::OrbitData() {

}

std::vector<SatelliteInfo> OrbitData::load() {
	std::vector<SatelliteInfo> data;

	if (isDataUpToDate())
		loadFromFile(data);
	else loadFromWeb(data);

	return data;
}

void OrbitData::loadFromFile(std::vector<SatelliteInfo>& outData) {
	...
	parse(buffer, length, outData);
	delete[] buffer;
}

void OrbitData::loadFromWeb(std::vector<SatelliteInfo>& outData) {
	std::cout << "Downloading data from http://" << DATA_URL << std::endl;

	Http http;
	HttpResponse response = http.request(DATA_URL);
	saveToFile(reinterpret_cast<const char*>(response.contents));

	parse(reinterpret_cast<const char*>(response.contents), sizeof(response.contents), outData);
}
...
tm readTime(std::string_view line) {
...
void OrbitData::parse(std::string_view data, size_t length, std::vector<SatelliteInfo>& outData) {
	...
	while (data.length() > 2) {
		...
		satellite.orbit = Orbit(sma, ecc, raan, incl, aop, trueAnomalyNow);

		outData.push_back(satellite);
	}
}
